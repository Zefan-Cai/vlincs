...
RUN_DIR="${RUN_DIR:-${REPO_ROOT}/local_runs/reproduce_visual_subcluster_ctf_top2_20260624}"
...
ASSIGNMENT_CSV="${PKG_DIR}/repro/input/rank02_visual_positive_subcluster_ctf_topk_source_assignments.csv"
P005_CONFIG="${PKG_DIR}/repro/input/p005_area_config.txt"
EXPECTED_P005="${PKG_DIR}/repro/expected/rank02_density_p005_area.json"
...
"${PYTHON_BIN}" "${REPO_ROOT}/kit/evaluate_sample_assignments_full.py" \
  ...
  --fallback singleton \
  --json "${RUN_DIR}/rank02_full_export.json" \
  --zip-out "${RUN_DIR}/rank02_full_export.zip"

echo "STAGE 2 density_simple_delivery"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/no_anchor_pervideo_filter_selector.py" \
  --source-zip "${RUN_DIR}/rank02_full_export.zip" \
  --policies density_simple \
  --json "${RUN_DIR}/rank02_density_simple.json" \
  --zip-out "${RUN_DIR}/rank02_density_simple.zip"

echo "STAGE 3 p005_area_delivery"
CONFIG="$(cat "${P005_CONFIG}")"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/evaluate_submission_detection_filter.py" \
  --submission-zip "${RUN_DIR}/rank02_density_simple.zip" \
  --config "${CONFIG}" \
  --json "${RUN_DIR}/rank02_density_p005_area.json" \
  --zip-out "${RUN_DIR}/rank02_density_p005_area.zip"

"${PYTHON_BIN}" - "${RUN_DIR}/rank02_full_export.json" "${RUN_DIR}/rank02_density_simple.json" "${RUN_DIR}/rank02_density_p005_area.json" "${EXPECTED_P005}" <<'PY'
import json
import sys
...
expected = primary(load(expected_path))

checks = [
    ("direct_idf1", direct["idf1"], 0.666140),
    ("density_idf1", density["idf1"], 0.668227),
