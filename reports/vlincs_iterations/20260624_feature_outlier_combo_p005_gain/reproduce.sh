...
RUN_DIR="${RUN_DIR:-${REPO_ROOT}/local_runs/reproduce_feature_outlier_combo_p005_20260624}"
...
ASSIGNMENT_CSV="${PKG_DIR}/repro/input/combo_01_03_04_05_feature_outlier_assignments.csv"
P005_CONFIG="${PKG_DIR}/repro/input/p005_area_config.txt"
EXPECTED_DIRECT="${PKG_DIR}/repro/expected/combo_01_03_04_05_full_export.json"
EXPECTED_DENSITY="${PKG_DIR}/repro/expected/combo_01_03_04_05_density_simple.json"
EXPECTED_P005="${PKG_DIR}/repro/expected/combo_01_03_04_05_density_p005_area.json"
...
"${PYTHON_BIN}" "${REPO_ROOT}/kit/evaluate_sample_assignments_full.py" \
  ...
  --fallback singleton \
  --json "${RUN_DIR}/combo_01_03_04_05_full_export.json" \
  --zip-out "${RUN_DIR}/combo_01_03_04_05_full_export.zip"

echo "STAGE 2 density_simple_delivery"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/no_anchor_pervideo_filter_selector.py" \
  --source-zip "${RUN_DIR}/combo_01_03_04_05_full_export.zip" \
  --policies density_simple \
  --json "${RUN_DIR}/combo_01_03_04_05_density_simple.json" \
  --zip-out "${RUN_DIR}/combo_01_03_04_05_density_simple.zip"

echo "STAGE 3 p005_area_delivery"
CONFIG="$(cat "${P005_CONFIG}")"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/evaluate_submission_detection_filter.py" \
  --submission-zip "${RUN_DIR}/combo_01_03_04_05_density_simple.zip" \
  --config "${CONFIG}" \
  --json "${RUN_DIR}/combo_01_03_04_05_density_p005_area.json" \
  --zip-out "${RUN_DIR}/combo_01_03_04_05_density_p005_area.zip"

"${PYTHON_BIN}" - "${RUN_DIR}/combo_01_03_04_05_full_export.json" "${RUN_DIR}/combo_01_03_04_05_density_simple.json" "${RUN_DIR}/combo_01_03_04_05_density_p005_area.json" "${EXPECTED_DIRECT}" "${EXPECTED_DENSITY}" "${EXPECTED_P005}" <<'PY'
import json
import sys
