...
RUN_DIR="${RUN_DIR:-${REPO_ROOT}/local_runs/reproduce_local_gated_subpart_combo_p005_20260624}"
...
EXPECTED_DIRECT="${PKG_DIR}/repro/expected/combo_top3_full_export.json"
EXPECTED_DENSITY="${PKG_DIR}/repro/expected/combo_top3_density_simple.json"
EXPECTED_P005="${PKG_DIR}/repro/expected/combo_top3_density_p005_area.json"
...
echo "STAGE 1 direct_full_export_from_cross_manifest_combo"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/evaluate_sample_assignments_full.py" \
  --tracklet-parquet "${tracklet_parquets[@]}" \
  --assignments "${LOCAL_COMBO_ASSIGNMENT}" \
  --fallback singleton \
  --json "${RUN_DIR}/combo_top3_full_export.json" \
  --zip-out "${RUN_DIR}/combo_top3_full_export.zip"

echo "STAGE 2 density_simple_delivery"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/no_anchor_pervideo_filter_selector.py" \
  --source-zip "${RUN_DIR}/combo_top3_full_export.zip" \
  --policies density_simple \
  --json "${RUN_DIR}/combo_top3_density_simple.json" \
  --zip-out "${RUN_DIR}/combo_top3_density_simple.zip"

echo "STAGE 3 p005_area_delivery"
CONFIG="$(cat "${P005_CONFIG}")"
"${PYTHON_BIN}" "${REPO_ROOT}/kit/evaluate_submission_detection_filter.py" \
  --submission-zip "${RUN_DIR}/combo_top3_density_simple.zip" \
  --config "${CONFIG}" \
  --json "${RUN_DIR}/combo_top3_density_p005_area.json" \
  --zip-out "${RUN_DIR}/combo_top3_density_p005_area.zip"

"${PYTHON_BIN}" - "${RUN_DIR}/combo_top3_full_export.json" "${RUN_DIR}/combo_top3_density_simple.json" "${RUN_DIR}/combo_top3_density_p005_area.json" "${EXPECTED_DIRECT}" "${EXPECTED_DENSITY}" "${EXPECTED_P005}" <<'PY'
import json
import sys
