#!/usr/bin/env python
"""Admission grid for one no-anchor Louvain resolver output.

This keeps the identity clustering fixed, then sweeps video-specific M3
admission thresholds. Ground truth is used only for post-hoc metrics/ranking.
"""
...
try:
    from kit.no_anchor_louvain_sweep import _louvain_labels
    ...
    from no_anchor_louvain_sweep import _louvain_labels
...
def _metric_tuple(row: dict[str, object], sort_key: str) -> tuple[float, float, float, float]:
    primary = float(row.get(sort_key, 0.0))
    return (
        primary,
...
def main() -> None:
    ...
    ap.add_argument("--edge-floor", type=float, default=0.04)
    ...
    labels, resolve_info = _louvain_labels(
        records,
        emb,
    ...
    for values in itertools.product(*[item[1] for item in grid]):
        ...
        row = {
            ...
            "output_tracklets": output_tracklets,
            "output_filtered_tracklets": int(output_info["output_filtered_tracklets"]),
            **metrics,
        }
        row["coverage_ratio"] = float(output_tracklets / max(len(records), 1))
        row["coverage_pair_score"] = float(row["tracklet_pair_f1"]) * min(1.0, row["coverage_ratio"])
        row["min_precision_recall"] = min(float(row["tracklet_pair_precision"]), float(row["tracklet_pair_recall"]))
        rows.append(row)

    rows.sort(key=lambda row: _metric_tuple(row, str(args.sort_key)), reverse=True)
