#!/usr/bin/env python
"""Export a no-anchor assignment CSV as a submission zip without scoring."""
...
from kit.evaluate_sample_assignments_full import _export_zip
from kit.no_anchor_resolve_sweep import _build_comp, _connect, _load_predictions


def _load_assignment_csv(path: Path, pred_col: str) -> tuple[dict[int, int], Counter[str]]:
    pred: dict[int, int] = {}
...
def main() -> None:
    ...
    pred_by_seq, statuses = _load_assignment_csv(assignment_csv, args.pred_col)
