...
def _attach_labels(candidates: list[dict[str, Any]], summary_rows: list[dict[str, Any]], baseline: float) -> list[dict[str, Any]]:
    ...
    for row in summary_rows:
        ...
        if not stem and (simple[0] or simple[1] or simple[2]):
    ...
    for cand in candidates:
        ...
        if not idf1_values:
            continue
        cand["_label_idf1"] = float(np.mean(idf1_values))
        cand["_label_delta_vs_baseline"] = float(cand["_label_idf1"] - baseline)
        cand["_label_count"] = int(len(idf1_values))
        cand["_label_summaries"] = sorted({str(row.get("_summary")) for row in matches})
        labeled.append(dict(cand))
    return labeled
