...
def weighted_label_propagation(
    ...
    Accepted weak edges are delivery-time positive evidence, so the resolver
    should form connected components unless a cannot-link or component-size
    guard blocks the merge.  A synchronous label-propagation update can
    oscillate on a two-node component; this greedy constrained union keeps the
    same deterministic surface while making each accepted edge monotonic.
    """
    ...
    for edge in sorted(accepted_edges, key=lambda e: (-float(e.score), e.left, e.right)):
        left, right = find(edge.left), find(edge.right)
        if left == right:
            continue
        merged = members[left] | members[right]
        if len(merged) > int(cfg.max_component_size):
            continue
        blocked = any(
            (min(a, b), max(a, b)) in cannot_link_pairs
    ...
    labels = np.array([find(i) for i in range(n_nodes)], dtype=np.int64)
